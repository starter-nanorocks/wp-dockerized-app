...
function blossom_coach_scripts(){
    ...
    if ( get_theme_mod( 'ed_localgoogle_fonts', false ) && ! is_customize_preview() && ! is_admin() && get_theme_mod( 'ed_preload_local_fonts', false ) ) {
        blossom_coach_preload_local_fonts( blossom_coach_fonts_url() );
    }
    ...
    if( blossom_coach_is_elementor_activated() ) {
        wp_enqueue_style( 'blossom-coach-elementor', get_template_directory_uri(). '/css' . $build . '/elementor' . $suffix . '.css', array(), BLOSSOM_COACH_THEME_VERSION );
    }
