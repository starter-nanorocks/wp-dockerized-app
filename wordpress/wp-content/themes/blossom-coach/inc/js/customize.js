jQuery(document).ready(function($){
    ...
    $('body').on('click', '#sub-accordion-panel-frontpage_settings .control-subsection .accordion-section-title', function(event) {
        ...
        Blossom_Coach_scrollToSection( section_id );
    });    

    $( 'input[name=blossom-coach-flush-local-fonts-button]' ).on( 'click', function( e ) {
        var data = {
            wp_customize: 'on',
            action: 'blossom_coach_flush_fonts_folder',
            nonce: blossom_coach_cdata.flushFonts
        };  
        $( 'input[name=blossom-coach-flush-local-fonts-button]' ).attr('disabled', 'disabled');

        $.post( ajaxurl, data, function ( response ) {
            if ( response && response.success ) {
                $( 'input[name=blossom-coach-flush-local-fonts-button]' ).val( 'Successfully Flushed' );
            } else {
                $( 'input[name=blossom-coach-flush-local-fonts-button]' ).val( 'Failed, Reload Page and Try Again' );
            }
        });
    });
});

function Blossom_Coach_scrollToSection( section_id ){
...
( function( api ) {
	...
	api.sectionConstructor['blossom-coach-pro-section'] = api.Section.extend( {
